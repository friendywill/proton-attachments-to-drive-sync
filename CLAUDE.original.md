...
I would like to build a simple system, where each attachment that I have ever
received in Proton mail, is synced to my Proton drive, so under proton drive,
it would exist under: Proton Drive > My Files > Proton Mail Attachments >
<email_address@example.com> > <Mail Subject (Number which increases based on
which email this is in the chain)> >  <attachment_name.pdf>.

If no attachments exist, don't do anything.

Where the <email_address@example.com> is a folder named after the email
address from the sender, <Mail Subject  (Number which increases based on
which email this is in the chain)> is a subfolder with the email subject name
and the number that is based on the number on which that email is in the
email chain, for example, if the email is the third response, the number
should be 3. <attachment_name.pdf> is just the original attachment name, if
there are multiple files, then multiple files should be in this folder.

I would also like a seperate folder for sent emails:
Proton Drive > My Files > Proton Mail Attachments > My Sent Emails > <Mail
Subject (Number which increases based on which email this is in the chain)> >
<attachment_name.pdf>.

## Requirements

- This applciation must have read only access to proton emails.
- Permissions to my Proton Drive must also be limited to a specific folder.
- Each component must run in a docker container, using docker compose.
- The check must happen once every minute.
- We can have multiple docker containers, EG one for mail bridge, and one for
  the app.

## Other instructions

- The preference is to use Python, if there is any deviation, express
  permission needs to be granted.
- Use a proper logger rather than prints.
...
The key words â€œMUSTâ€, â€œMUST NOTâ€, â€œREQUIREDâ€, â€œSHALLâ€, â€œSHALL NOTâ€, â€œSHOULDâ€,
...
interpreted as described in RFC 2119.
  ...
  9.  A footerâ€™s token MUST use - in place of whitespace characters, e.g.,
  ...
  10. A footerâ€™s value MAY contain spaces and newlines, and parsing MUST
...
characters, a new line must be created to continue the description.
